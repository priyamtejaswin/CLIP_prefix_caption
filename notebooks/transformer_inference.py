...
# # Running for TextVQA images.

# In[13]:


def clip_caption(filepath):
    try:
...
assert os.path.exists(args.outpath) is False, "--outpath already exists. Give a new one."
assert os.path.isdir(args.images), "--images is not a dir."

fnames = [name for name in os.listdir(args.images) if name.endswith('.jpg') or name.endswith('.jpeg') or name.endswith('.png')]
assert len(fnames) > 0, "No images found in --images."
failed = 0
mclips = []
for name in tqdm(fnames):
    path = os.path.join(args.images, name)
    talk = clip_caption(path)
    if talk is False:
        failed += 1
    else:
        mclips.append(
...
with open(args.outpath, 'w') as fp:
    fp.write(json.dumps(mclips))

print("Done. Failed to load images for", failed)
print("Clip captions for each image are saved in", args.outpath)
